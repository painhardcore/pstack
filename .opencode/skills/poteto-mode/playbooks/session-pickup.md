...
**You own the resume point. Read the prior trail, don't redo it.** Use for "take over this", "resume this conversation", "continue from this task", or a pushed branch that the user wants continued.
...
1. Locate the prior trail through the active host's task history, a user-provided transcript, a decision log, or a pushed branch. Read the overview and latest messages first, then scan back for decisions. If delegation is available and permitted, use a read-only subagent for a long transcript. Otherwise reduce it inline. Never scan unrelated projects or private chats.

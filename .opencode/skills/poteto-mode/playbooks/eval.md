...
6. **Verify the chain from task history, not self-report.** Use the active host's task transcript or tool history when available. Otherwise inspect the files, commands, and artifacts the candidate produced. Never scan unrelated projects. Citing a principle is not reading its leaf skill, and reading it is not applying it.

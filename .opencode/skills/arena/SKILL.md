...
After all Phase B candidates complete, use a read-only judge on a different available model family when the host supports it. Otherwise judge inline. The judge sees the rubric and candidates by path label, scores each criterion, and recommends a base with rationale. Never start judging while candidates are still writing.

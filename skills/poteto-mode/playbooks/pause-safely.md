...
**You own a clean stop. Leave a checkpoint a cold-start agent can resume from.** Use for "pause safely", "I need to go offline", an agent restart, or imminent context compaction. This is explicit only. On "keep going" or "don't stop", continue. Autonomous run already checkpoints each iteration.

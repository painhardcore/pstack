...
2. Binary-search the cause. Form candidate hypotheses, then rule them out until one survives. Seed them with **how** over the affected subsystem and **why** for regression history. Each pass takes the split that removes the most remaining problem space. Add instrumentation when state is unclear. Repeat until runtime evidence confirms the surviving mechanism.
3. Plan the fix. If it crosses a function boundary, run **architect** first. Delegate with a specific scope when subagents are available and permitted. Otherwise implement inline. Review the actual diff either way.

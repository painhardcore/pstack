...
Before starting investigators, inspect the active host's available tools, connectors, and MCP servers. Do not search the public web for private workspace evidence.
...
When the host supports subagents and the user permits them, launch all matching investigators together. One investigator owns one evidence category. Inherit the parent model. Give each investigator read access to its assigned connector without granting write actions. If subagents are unavailable or forbidden, query the same categories inline and keep the same coverage map.
...
Use one synthesizer subagent when delegation is available. Inherit the parent model and grant read-only connector access so it can spot-check citations. Otherwise synthesize inline.

...
0. **Locate the target.** Read [the host-path table](../create-verification-skill/references/host-paths.md), then find `verify-*` under the active host's project skill root. Confirm that the candidate has launch and drive sections plus a feature map. If several candidates exist, ask which one. If none exist, stop and point at **create-verification-skill** instead of inventing a target.
...
2. **Source wave.** When subagents are available and permitted, launch one read-only subagent per feature file. Otherwise inspect every feature inline. Each pass explains how the user-facing feature works from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Source readers never drive the app or edit files. Return the feature summary, source entry points, likely drift or none, and one recipe.
